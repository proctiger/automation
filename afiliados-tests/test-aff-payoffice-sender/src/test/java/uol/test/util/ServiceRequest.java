...
public class ServiceRequest {
    ...
    public Status doPost(String url, MediaType mediaType) throws Exception {
        ...
        try {
            String type;
            if (mediaType != null) {
                System.out.println(String.format("Content-Type:%s", type = mediaType.toString()));
            } else {
                System.out.println(String.format("Content-Type:%s", type = "text/plain"));
            }
            addParams(request);
    ...
    public String doPostReturnBody(String url, MediaType mediaType) throws Exception {
        ClientRequest request = createClientRequest(url);
        ClientResponse<String> result = null;
        try {
            String type;
    ...
    private <T> T doHttpMethod(String url,
        ...
        try {
            ...
            if (result.getResponseStatus() == Response.Status.OK || result.getResponseStatus() == Response.Status.ACCEPTED) {
                T entity;
                System.out.printf("O servico '{}' respondeu com a seguinte mensagem: {}", url, (entity = result.getEntity(clazz)).toString());
                return entity;
            }
        } catch (Exception e) {
        } finally {
